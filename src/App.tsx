...
// CRITICAL: Mock functions should ONLY be used when NO real LSL data is available
const mockInvoke = async (command: string, args?: any) => {
  ...
  switch (command) {
    case 'connect_to_lsl_stream':
      ...
      // IMPORTANT: This should FAIL if no real LSL stream is available
      // Only succeed for testing purposes
      if (streamName === '123' || streamName.includes('Unicorn') || streamName.includes('unicorn')) {
        const result = {
          name: streamName,
          channel_count: 17,
          sample_rate: 250.0,
          is_connected: false, // CHANGED: Mark as simulated, not real connection
          metadata: "⚠️ SIMULATED DATA - Type: EEG | Source: MockUnicorn_12345 | Channels: 17 | Rate: 250.0 Hz | Manufacturer: Mock g.tec | Model: Simulated Unicorn",
          stream_type: "EEG_SIMULATED",
          source_id: "MockUnicorn_12345",
          channel_names: ["Fz", "C3", "Cz", "C4", "Pz", "PO7", "Oz", "PO8", "ACC_X", "ACC_Y", "ACC_Z", "GYR_X", "GYR_Y", "GYR_Z", "Battery", "Counter", "Validation"],
          manufacturer: "Mock g.tec medical engineering GmbH",
          device_model: "Simulated Unicorn Hybrid Black"
        };
        console.log('⚠️ Mock connection (SIMULATED DATA):', result);
        return result;
      } else {
        const error = `❌ No real LSL stream found with name: '${streamName}'. This is expected in demo mode - real EEG device required for live data.`;
        console.log('❌ Mock connection failed:', error);
        throw new Error(error);
      }
      
    case 'start_eeg_processing':
      console.log('🚀 Mock starting EEG processing (SIMULATED DATA)');
      return { success: true };
...
const invoke = async (command: string, args?: any) => {
  try {
    if (isTauri && window.__TAURI__?.invoke) {
      ...
      console.log('⚠️ Using mock invoke (SIMULATED DATA ONLY):', command, args);
...
function App() {
  ...
  const handleConnectToLSL = async () => {
    ...
    try {
      ...
      console.log(`🔧 Environment: ${isTauri ? 'REAL Tauri (can get real LSL data)' : 'Web browser (simulated data only)'}`);
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {connectionStatus === 'connected' && (
                        <>
                          ...
                          <div>
                            <p className="font-bold text-[#0B3142] text-base">
                              {streamInfo?.is_connected ? 'Real neural interface established!' : 'Simulated neural interface ready!'}
                            </p>
                            <p className="text-sm text-[#0B3142]/70">
                              {streamInfo?.is_connected ? 'Receiving live EEG data...' : 'Generating realistic simulated data...'}
                            </p>
                          </div>
                        </>
                      )}
